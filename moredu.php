...
if ($num_rows != 0) {
    ...
    if (strlen($out_text) + strlen($add_on_top) + strlen($addon) > 320) {
        //$has_options = false;
        if ($has_options) {
            $addon = "\n1. View more results";
            if (!$free) {
        ...
        do {
            $length_needed = 320 - ( strlen($sents) + strlen($addon) + strlen($add_on_top) );
...
//            }
        ...
        } while (strlen($sents) + strlen($addon) + strlen($add_on_top) + $cut < 320);
